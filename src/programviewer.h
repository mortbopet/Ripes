...
public:
  ...
  const Assembler::AddrOffsetMap &addressOffsetMap() const {
    return m_labelAddrOffsetMap;
  }

  ///
  /// \brief updateProgram
  /// Refreshes the programViewer view, using the parser to either disassemble
  /// or (if @p binary is true) show the raw binary version of the loaded
  /// program.
  ///
...
private:
   ...
   * If program following is enabled, this function will ensure that the current
   * instruction in the first stage of the processor is always visible within
   * the program viewer.
   */
   ...
   * To correctly correlate a line index with program address location whilst
   * accounting for additional output in the disassembled view of a program, we
   * provide the AddrOffsetMap. Key in this map indicate line numbers (starting
   * from 0) which does >not< correspond with an address. The value of the key
   * corresponds to the total sum of non-address lines encountered up to and
...
public:
  ...
  QPixmap m_breakpoint =
      QPixmap(":/icons/breakpoint_enabled.png").scaled(imageWidth, imageHeight);
  QPixmap m_breakpoint_disabled =
      QPixmap(":/icons/breakpoint_disabled.png").scaled(imageWidth, imageWidth);

  void setBlockHeight(int height) { m_blockHeight = height; }

protected:
  void paintEvent(QPaintEvent *event) override {
    m_programViewer->breakpointAreaPaintEvent(event);
  }
...
private:
  ...
  void wheelEvent(QWheelEvent *event) override {
    m_programViewer->verticalScrollBar()->setValue(
        m_programViewer->verticalScrollBar()->value() +
