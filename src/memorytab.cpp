...
void MemoryTab::saveAddress() {
    ...
    dialog.setLabelText(
      QString("Please enter a label for address: %1")
        .arg(QString("0x%1").arg(
          QString().setNum(m_model->getCentralAddress(), 16).rightJustified(8, '0'))));
...
void MemoryTab::updateRegisterWidget(int n) {
    // Invoked when runner edits an internal register, requiring an update of
    // the
    // widget
