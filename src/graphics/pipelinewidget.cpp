...
PipelineWidget::PipelineWidget(QWidget* parent) : QGraphicsView(parent) {
    ...
    Graphics::Shape* instr_mem = new Graphics::Shape(
      Graphics::ShapeType::Block, Graphics::Stage::IF, 60, 0);
    ...
    // State registers
    ifid = new Graphics::Shape(Graphics::ShapeType::Block, Graphics::Stage::ID,
                               0, 10);
    ifid->setName("IF/ID");
    ...
    idex = new Graphics::Shape(Graphics::ShapeType::Block, Graphics::Stage::EX,
                               0, 10);
    idex->setName("ID/EX");
    ...
    Graphics::Text* if_instr = new Graphics::Text(QPointF(
      -dash1->sceneBoundingRect().left() +
        dash1->sceneBoundingRect().width() / 2 + -spaceBetweenStateRegs / 2,
      dash1->sceneBoundingRect().top() + textPad));
    Graphics::Text* id_instr = new Graphics::Text(QPointF(
      dash1->sceneBoundingRect().left() +
        dash1->sceneBoundingRect().width() / 2 + spaceBetweenStateRegs / 2,
      dash1->sceneBoundingRect().top() + textPad));
    Graphics::Text* ex_instr = new Graphics::Text(QPointF(
      dash2->sceneBoundingRect().left() +
        dash2->sceneBoundingRect().width() / 2 + spaceBetweenStateRegs / 2,
      dash2->sceneBoundingRect().top() + textPad));
    Graphics::Text* mem_instr = new Graphics::Text(QPointF(
      dash3->sceneBoundingRect().left() +
        dash3->sceneBoundingRect().width() / 2 + spaceBetweenStateRegs / 2,
      dash3->sceneBoundingRect().top() + textPad));
    Graphics::Text* wb_instr = new Graphics::Text(
      QPointF(dash4->sceneBoundingRect().left() + spaceBetweenStateRegs / 3,
