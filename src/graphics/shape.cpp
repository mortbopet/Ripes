...
QPointF Shape::getInputPoint(int index) const {
  if (index >= m_inputPoints.size()) return QPointF();
  return m_inputPoints[index];
}

QPointF Shape::getOutputPoint(int index) const {
  if (index >= m_outputPoints.size()) return QPointF();
