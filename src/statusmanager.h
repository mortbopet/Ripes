...
public:
  void setStatus(const QString &s) {
    // Ensure that status messages are not multiline - this will expand the
    // bottom status bar in the main window, which we don't want.
...
// Helper class to create StatusManagerBase singletons based on a statically
// defined string.
...
/**
 * @brief StatusManager
 * Defines a singleton class which may be used to send or clear a message to a
 * specific label within the main window status bar. Each status manager is a
 * singleton based on the status manager name.
 */
template <typename T>
...
public:
  ...
  // Pushes a message to the message queue of this status manager. If ms is set
  // and >0, the message will be removed after ms milliseconds.
  static void setStatusTimed(const QString &s, int ms = 1000) {
    auto &me = get();
    assert(!me.isPermanent &&
           "Cannot set timed messages on permanent status managers.");
    me.statusQueue.insert(0, TimedMessage());
    auto &message = me.statusQueue[0];
    message.message = s;
    message.time = ms;
...
private:
  ...
  StatusManagerBase() {
    ...
    m_timer.connect(&m_timer, &QTimer::timeout,
                    [this] { this->clearStatus(); });
  }
  QTimer m_timer;
  QList<TimedMessage> statusQueue;

  // If true, does not allow timed status messages and disables the status
  // queue.
  bool isPermanent;
};
