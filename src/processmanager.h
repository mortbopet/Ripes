...
private:
  // Tries to set the program from a set of optional default program names.
  // These program names are expected to be found in the PATH.
  ...
  bool initialize() {
    ...
    if (!settingsPath.isEmpty() && RipesSettings::hasSetting(settingsPath)) {
      // This program has a settings path associated with it. Link up with the
      // settings path, ensuring that we update and revalidate the program on
      // each settings path change.
      auto *observer = RipesSettings::getObserver(settingsPath);
      observer->connect(
          observer, &SettingObserver::modified,
  ...
  ProcessResult _run(const QStringList &args, bool showProgressDialog) {
    ...
    QProgressDialog progressDiag =
        QProgressDialog("Executing program...", "Abort", 0, 0, nullptr);
    m_process.connect(&progressDiag, &QProgressDialog::canceled, &m_process,
                      &QProcess::kill);
    m_process.connect(&progressDiag, &QProgressDialog::canceled, &m_process,
                      [this] { m_aborted = true; });
    m_process.connect(
    ...
    m_process.connect(&m_process, &QProcess::errorOccurred, &progressDiag,
                      &QProgressDialog::reset);
    m_process.connect(&m_process, &QProcess::errorOccurred,
                      [this]() { m_errored = true; });
    m_process.setWorkingDirectory(
        static_cast<Impl *>(this)->getWorkingDirectory());
     ...
     * not remove the race condition. Such race condition seems inherintly tied
     * to how QDialog::exec works and no proper fix has been able to be found
     * (yet).*/
    if (!m_errored && showProgressDialog) {
      progressDiag.exec();
    }
    m_process.waitForFinished();
    return ProcessResult{QString(m_process.readAllStandardOutput()),
