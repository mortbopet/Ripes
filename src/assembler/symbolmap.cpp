...
std::optional<Error> SymbolMap::addAbsSymbol(const unsigned& line, const Symbol& s, VInt v) {
    if (abs.count(s)) {
...
std::optional<Error> SymbolMap::addRelSymbol(const unsigned& line, const Symbol& s, VInt v) {
    assert(s.isLocal());
    auto& it = rel[s.v.toInt()];
    if (it.count(line))
        return {Error(line, QString::fromStdString("Multiple definitions of relative symbol '" + std::to_string(v) +
                                                   "' on line '" + std::to_string(line)))};
    it[line] = v;
    return {};
}

AbsoluteSymbolMap SymbolMap::copyRelativeTo(unsigned line, const QString& beforeSuffix,
