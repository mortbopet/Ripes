...
public:
     ...
     */
    using PseudoOpFunctorRetT = std::optional<std::vector<Assembler::LineTokens>>;
    using PseudoOpFunctor = std::function<std::variant<Assembler::Error, PseudoOpFunctorRetT>(const SourceLine&)>;
    ...
    using AssemblerFunctor = std::function<std::variant<Assembler::Error, AssemblerFunctorRetT>(
        const SourceLine&, const Assembler::SymbolMap&)>;
