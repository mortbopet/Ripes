...
protected:
  virtual bool
  isValidWayInstruction(const AInt instructionIndex,
                        const TokenizedSrcLine &line, const InstrRes &assembled,
    ...
    error = "Unrecognized Way Instruction, expected an <b>" + wayName +
            "</b>-Way instruction but got instruction '<b>" + instr->name() +
            "</b>'";
    return false;
  }
...
private:
  ...
  bool
  isDataWayInstruction(const std::shared_ptr<InstructionBase> instr) const {
    ...
    return dataWayInstructions.count(
               instr->getOpPart(0 /*get opPartOpcode*/)) != 0;
  }
