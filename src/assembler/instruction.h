...
//    * @param fields: list of OpParts corresponding to the identifying elements
//    of
//    * the opcode.
//    */
...
//   /**
//    * @brief Reg
//    * @param tokenIndex: Index within a list of decoded instruction tokens
//    that
...
//  /**
//   * @brief Imm
//   * @param _tokenIndex: Index within a list of decoded instruction tokens
//   that
...
//   * immediate
//   * @param _symbolType: Set if this immediate refers to a relative or
//   absolute
//   * symbol.
//   * @param _symbolTransformer: Optional function used to process the
//   immediate
...
//    }
//    if (repr == Repr::Signed) {
//      line.push_back(QString::number(vsrtl::signextend(reconstructed,
//      width)));
...
//       for (const auto &field : _this->m_fields) {
//         if (auto error = field->decode(instruction, address, symbolMap,
//         line)) {
...
//    // Sort fields by token index. This lets the disassembler naively write
//    each
//    // parsed field in the order of m_fields, ensuring that the disassembled
//    // tokens appear in correct order, without having to manually check this
//    on
...
//  Result<LineTokens> disassemble(const Instr_T instruction, const Reg_T
//  address,
...
//    // 2.
//    assert(registeredBits.count(0) == 1 && "Expected bit 0 to be in
//    bit-range");
...
//    // 3.
//    assert(nBits % CHAR_BIT == 0 && "Expected instruction to be
//    byte-aligned"); m_byteSize = nBits / CHAR_BIT;
//  }
