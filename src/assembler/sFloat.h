...
extern "C" {
/* arm_neon.h type conflict as described in
 * https://github.com/ucb-bar/berkeley-softfloat-3/issues/12 and workaround as
 * described in
 * https://github.com/SDL-Hercules-390/hyperion/issues/675#issuecomment-2248692596
 */
...
public:
  ...
  // member functions
  inline uint8_t sign() const { return signF32UI(word); }
  inline uint8_t exponent() const { return expF32UI(word); }
  inline uint32_t fraction() const { return fracF32UI(word); }
  inline Parts parts() const {
    return {.fraction = fraction(), .exponent = exponent(), .sign = sign()};
  }
  ...
  static inline Float32_t fma(const Float32_t a, const Float32_t b,
                              const Float32_t c,
                              const fma_mode mode = fma_mode::add_AB_add_C) {
    return Float32_t{.softfloat32 = softfloat_mulAddF32(
                         a.softfloat32.v, b.softfloat32.v, c.softfloat32.v,
...
protected:
  ...
  static Float32_t minMaxComp(const Float32_t a, const Float32_t b,
                              bool isMax) {
...
template <>
inline Float32_t Float32_t::from<int32_t>(int32_t convert_int) {
  return Float32_t{.softfloat32 = i32_to_f32(convert_int)};
}
template <>
inline Float32_t Float32_t::from<uint32_t>(uint32_t convert_int) {
  return Float32_t{.softfloat32 = ui32_to_f32(convert_int)};
}
template <>
inline Float32_t Float32_t::from<int64_t>(int64_t convert_int) {
  return Float32_t{.softfloat32 = i64_to_f32(convert_int)};
}
template <>
inline Float32_t Float32_t::from<uint64_t>(uint64_t convert_int) {
  return Float32_t{.softfloat32 = ui64_to_f32(convert_int)};
}
template <>
inline Float32_t Float32_t::from<float>(float convert_float) {
  return Float32_t{.float32 = convert_float};
}

template <>
inline int32_t Float32_t::to<int32_t>(Float32_t float_val) {
  return f32_to_i32(float_val.softfloat32, softfloat_roundingMode, true);
}
template <>
inline uint32_t Float32_t::to<uint32_t>(Float32_t float_val) {
  return f32_to_ui32(float_val.softfloat32, softfloat_roundingMode, true);
}
template <>
inline int64_t Float32_t::to<int64_t>(Float32_t float_val) {
  return f32_to_i64(float_val.softfloat32, softfloat_roundingMode, true);
}
template <>
inline uint64_t Float32_t::to<uint64_t>(Float32_t float_val) {
  return f32_to_ui64(float_val.softfloat32, softfloat_roundingMode, true);
}
template <>
inline float Float32_t::to<float>(Float32_t float_val) {
