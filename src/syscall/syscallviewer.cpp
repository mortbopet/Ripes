...
SyscallViewer::SyscallViewer(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::SyscallViewer) {
  m_ui->setupUi(this);
  ...
  m_ui->syscallList->setHorizontalHeaderLabels(
      {"Func. (" +
           ProcessorHandler::currentISA()->gprRegInfo()->regAlias(
  ...
  m_ui->syscallArguments->setHorizontalHeaderLabels(
      {"Arg. #", "Reg.", "Description"});
  ...
  m_ui->syscallReturns->setHorizontalHeaderLabels(
      {"Ret. #", "Reg.", "Description"});
...
void SyscallViewer::handleItemChanged(QTableWidgetItem *current,
                                      QTableWidgetItem *) {
  if (current == nullptr)
    return;
...
void SyscallViewer::addItemToTable(QTableWidget *table, unsigned int idx,
  ...
  auto gpr = isa->gprRegInfo();

  idxItem->setData(Qt::EditRole, QString::number(idx));
  regItem->setData(Qt::EditRole, gpr->regAlias(isa->syscallArgReg(idx)));
...
SyscallViewer::~SyscallViewer() { delete m_ui; }
