...
void ctkRangeSliderPrivate::init() {
  ...
  q->connect(q, SIGNAL(rangeChanged(int, int)), q,
             SLOT(onRangeChanged(int, int)));
}

// --------------------------------------------------------------------------
ctkRangeSliderPrivate::Handle
ctkRangeSliderPrivate::handleAtPos(const QPoint &pos, QRect &handleRect) const {
...
int ctkRangeSliderPrivate::pixelPosToRangeValue(int pos) const {
  ...
  QRect gr = q->style()->subControlRect(QStyle::CC_Slider, &option,
                                        QStyle::SC_SliderGroove, q);
  QRect sr = q->style()->subControlRect(QStyle::CC_Slider, &option,
...
int ctkRangeSliderPrivate::pixelPosFromRangeValue(int val) const {
  ...
  return QStyle::sliderPositionFromValue(q->minimum(), q->maximum(), val,
                                         sliderMax - sliderMin,
                                         option.upsideDown) +
         sliderMin;
}
...
ctkRangeSlider::ctkRangeSlider(QWidget *_parent)
    : QSlider(_parent), d_ptr(new ctkRangeSliderPrivate(*this)) {
  Q_D(ctkRangeSlider);
  d->init();
}
...
ctkRangeSlider::ctkRangeSlider(Qt::Orientation o, QWidget *parentObject)
    : QSlider(o, parentObject), d_ptr(new ctkRangeSliderPrivate(*this)) {
  Q_D(ctkRangeSlider);
  d->init();
}
...
ctkRangeSlider::ctkRangeSlider(ctkRangeSliderPrivate *impl, Qt::Orientation o,
    ...
    : QSlider(o, parentObject), d_ptr(impl) {
  Q_D(ctkRangeSlider);
  d->init();
}
...
void ctkRangeSlider::setValues(int l, int u) {
  ...
  if (emitMinPosChanged || emitMaxPosChanged || emitMinValChanged ||
      emitMaxValChanged) {
    this->update();
  }
}
...
void ctkRangeSlider::paintEvent(QPaintEvent *) {
  ...
  const QRect lr = style()->subControlRect(QStyle::CC_Slider, &option,
                                           QStyle::SC_SliderHandle, this);
  option.sliderPosition = d->m_MaximumPosition;

  const QRect ur = style()->subControlRect(QStyle::CC_Slider, &option,
                                           QStyle::SC_SliderHandle, this);

  QRect sr = style()->subControlRect(QStyle::CC_Slider, &option,
  ...
  // Render the range
  //
  QRect groove = this->style()->subControlRect(QStyle::CC_Slider, &option,
                                               QStyle::SC_SliderGroove, this);
...
void ctkRangeSlider::mousePressEvent(QMouseEvent *mouseEvent) {
  ...
  int mepos = this->orientation() == Qt::Horizontal ? mouseEvent->pos().x()
                                                    : mouseEvent->pos().y();
  ...
  QRect handleRect;
  ctkRangeSliderPrivate::Handle handle_ =
      d->handleAtPos(mouseEvent->pos(), handleRect);
  ...
  if (handle_ != ctkRangeSliderPrivate::NoHandle) {
    ...
    d->m_SubclassClickOffset =
        mepos - (this->orientation() == Qt::Horizontal ? handleRect.left()
                                                       : handleRect.top());
  ...
  QRect sr = style()->subControlRect(QStyle::CC_Slider, &option,
                                     QStyle::SC_SliderGroove, this);
  int minCenter = (this->orientation() == Qt::Horizontal ? handleRect.left()
                                                         : handleRect.top());
  int maxCenter = (this->orientation() == Qt::Horizontal ? handleRect.right()
                                                         : handleRect.bottom());
  if (control == QStyle::SC_SliderGroove && mepos > minCenter &&
      mepos < maxCenter) {
    // warning lost of precision it might be fatal
    d->m_SubclassPosition = (d->m_MinimumPosition + d->m_MaximumPosition) / 2.;
    d->m_SubclassClickOffset =
        mepos - d->pixelPosFromRangeValue(d->m_SubclassPosition);
    d->m_SubclassWidth = (d->m_MaximumPosition - d->m_MinimumPosition) / 2;
    qMax(d->m_SubclassPosition - d->m_MinimumPosition,
         d->m_MaximumPosition - d->m_SubclassPosition);
    this->setSliderDown(true);
    if (!this->isMinimumSliderDown() || !this->isMaximumSliderDown()) {
      d->m_SelectedHandles = QFlags<ctkRangeSliderPrivate::Handle>(
                             ...
                             QFlags<ctkRangeSliderPrivate::Handle>(
                                 ctkRangeSliderPrivate::MaximumHandle);
...
void ctkRangeSlider::mouseMoveEvent(QMouseEvent *mouseEvent) {
  ...
  const int m =
      style()->pixelMetric(QStyle::PM_MaximumDragDistance, &option, this);
...
// --------------------------------------------------------------------------
void ctkRangeSlider::initMinimumSliderStyleOption(
    QStyleOptionSlider *option) const {
  this->initStyleOption(option);
}

// --------------------------------------------------------------------------
void ctkRangeSlider::initMaximumSliderStyleOption(
    QStyleOptionSlider *option) const {
  this->initStyleOption(option);
}
