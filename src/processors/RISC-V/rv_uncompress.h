...
/**
 * @brief uncompressInstruction
 * Expands a (possibly compressed, 'C' extension) instruction word into its
 * equivalent 32-bit representation. This is the pure, reusable core of the
 * decompression logic - it is used both by the Uncompress hardware component
 * and by the software ISA-simulator processor model.
 *
 * The @p cEnabled and @p rv64 flags are passed in (rather than queried from an
 * ISAInfoBase) so this stays allocation-free and cheap enough to call on every
 * fetched instruction.
 *
 * @param instrValue raw instruction word (a 16-bit compressed instruction
 *        occupies the lower halfword).
 * @param cEnabled whether the 'C' extension is enabled. If false, no
 *        decompression is performed.
 * @param rv64 whether the target is RV64 (selects the RV64-only compressed
 *        encodings such as c.ld / c.sd).
 * @param pcInc4 [out] set to true if the instruction is a regular 32-bit
 *        instruction (PC should advance by 4), false if it is a 16-bit
 *        compressed instruction (PC should advance by 2).
 * @return the uncompressed 32-bit instruction word.
 */
inline VInt uncompressInstruction(VInt instrValue, bool cEnabled, bool rv64,
                                  bool &pcInc4) {
  const bool disabled = !cEnabled;
  pcInc4 = disabled || ((instrValue & 0b11) == 0b11) || (instrValue == 0);
  if (disabled || ((instrValue & 0b11) == 0b11)) {
    // Not a compressed instruction (or 'C' unsupported): pass through.
    return instrValue;
  }
  ...
  case 0x00: // quadrant
    ...
    case 0b000: {       // c.addi4spn
      if (instrValue) { // not illegal instruction
        const auto fields =
            RVInstrParser::getParser()->decodeCIW16Instr(instrValue);
        rd = fields[3] | 0x8;
        uimm = (((fields[2] & 0x3C) << 2) | ((fields[2] & 0xC0) >> 4) |
                ((fields[2] & 0x01) << 1) | ((fields[2] & 0x02) >> 1))
               << 2;
        // addi rd ′ , x2, nzuimm[9:2]
        new_instr = (uimm << 20) | (0b00010 << 15) | (0b000 << 12) | (rd << 7) |
    ...
    case 0b110: // c.sw
    {
      ...
      new_instr = (((uimm & 0xFE0) >> 5) << 25) | (rs2 << 20) | (rs1 << 15) |
                  (0b010 << 12) | ((uimm & 0x1F) << 7) | RVISA::OpcodeID::STORE;
    } break;
    case 0b111:
      if (rv64) { // c.sd
        const auto fields =
  ...
  case 0x01: // quadrant
    ...
    {
      const auto fields =
          RVInstrParser::getParser()->decodeCA16Instr(instrValue);
      rd = fields[4] | 0x8;
      rs2 = fields[6] | 0x8;
      switch (fields[3]) {
      case 0b00: { // c.srli
        const auto fieldscb =
            RVInstrParser::getParser()->decodeCB216Instr(instrValue);
    ...
    case 0b110: { // c.beqz
      const auto fields =
          RVInstrParser::getParser()->decodeCB16Instr(instrValue);
      rs1 = fields[3] | 0x8;
  ...
  case 0x02: // quadrant
    ...
    case 0b010: { // c.lwsp
      ...
      // lw rd,offset[7:2](x2)
      new_instr = (uimm << 20) | (0b0010 << 15) | (0b010 << 12) | (rd << 7) |
                  RVISA::OpcodeID::LOAD;
    } break;
    case 0b011:
      if (rv64) { // c.ldsp
        ...
        rd = fields[3];
        uimm =
            ((fields[4] & 0x07) << 6) | (fields[2] << 5) | (fields[4] & 0x18);
        // ld rd,offset[8:3](x2)
        new_instr = (uimm << 20) | (0b0010 << 15) | (0b011 << 12) | (rd << 7) |
                    RVISA::OpcodeID::LOAD;
      }
    ...
    case 0b100: {
      ...
      if (fields[2]) {
        ...
        if (rs2) { // c.mv
                   // add rd, x0, rs2
          new_instr = (rs2 << 20) | (0b0 << 15) | (0b000 << 12) | (rd << 7) |
                      RVISA::OpcodeID::OP;
        } else { // c.jr
          // jalr x0, 0(rs1)
          new_instr = (0b0 << 20) | (rd << 15) | (0b000 << 12) |
...
public:
  void setISA(const std::shared_ptr<ISAInfoBase> &isa) {
    m_cEnabled = isa->extensionEnabled("C");
    m_rv64 = isa->isaID() == ISA::RV64I;
  }
  ...
  Uncompress(std::string name, SimComponent *parent) : Component(name, parent) {
    ...
    Pc_Inc << [this] {
      bool pcInc4 = true;
      uncompressInstruction(instr.uValue(), m_cEnabled, m_rv64, pcInc4);
      return pcInc4;
    };
    ...
    exp_instr << [this] {
      bool pcInc4 = true;
      return uncompressInstruction(instr.uValue(), m_cEnabled, m_rv64, pcInc4);
    };
  }
