  ...
  BranchMiss(const std::string &name, SimComponent *parent)
    ...
    curr_miss_1 << [=] {
      return ( curr_is_j.uValue()     && !curr_equal_targets.uValue())
           | (!curr_pre_take.uValue() &&  curr_act_take.uValue() && curr_is_b.uValue())
           | ( curr_pre_take.uValue() &&  curr_act_take.uValue() && !curr_equal_targets.uValue() && curr_is_b.uValue());
    };

    curr_miss_2 << [=] {
      return (!curr_act_take.uValue() && curr_pre_take.uValue() && curr_is_b.uValue());
    };
  }
