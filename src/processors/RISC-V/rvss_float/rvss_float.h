...
public:
  RVSS_FLOAT(const QStringList &extensions)
      : RipesVSRTLProcessor("Single Cycle RISC-V floating point Processor") {
    
    QStringList exts{extensions};
    if(!exts.contains(Extension::F.name))
      exts << Extension::F.name; // enforce F extension
  ...
  static ProcessorISAInfo supportsISA() { 
    return ProcessorISAInfo{
      ...
      {Extension::M.name, Extension::F.name, Extension::C.name},
      {Extension::M.name, Extension::F.name}
    };
  }
