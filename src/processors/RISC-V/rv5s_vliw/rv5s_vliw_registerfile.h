...
class RegisterFile_VLIW : public Component {
public:
  SetGraphicsType(ClockedComponent);
  RegisterFile_VLIW(const std::string &name, SimComponent *parent)
  ...
  SUBCOMPONENT(rf_1, TYPE(RegisterFile<XLEN, true>));
  SUBCOMPONENT(rf_2, TYPE(RegisterFile<XLEN, true>));
  ...
  VSRTL_VT_U getRegister(unsigned i) {
    return m_memory->readMem(i << ceillog2(XLEN / CHAR_BIT), XLEN / CHAR_BIT);
  }

private:
  /**
   * Extra level of read bypassing to handle the fact that the RegisterFile
   * class only bypasses with its internal signals, and is unaware of the
   * dual-ported register file.
   * Assuring that data way value is being preferred
   */
  VSRTL_VT_U doReadBypass(const VSRTL_VT_U reg_idx, const int portIndex,
                          const RegisterFile<XLEN, true> *rf) {
    if (reg_idx == 0)
