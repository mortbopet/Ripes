...
PicoRV32::PicoRV32(const QStringList&) : m_dataAccess(), m_instrAccess() {
    m_features = 0;
    top = new Vpicorv32;
    m_isa = std::make_shared<ISAInfo<ISA::RV32I>>(QStringList({"M"}));
}
...
void PicoRV32::resetProcessor() {
    ...
    // programming easier.
    top->rootp->picorv32__DOT__reg_next_pc = m_initialPC;
    top->rootp->picorv32__DOT__reg_pc = m_initialPC;
...
StageInfo PicoRV32::stageInfo(unsigned /*stageIndex*/) const {
    ...
    StageInfo si;
    State state = static_cast<State>(top->rootp->picorv32__DOT__cpu_state);
...
unsigned int PicoRV32::getPcForStage(unsigned /*stageIndex*/) const {
    return top->rootp->picorv32__DOT__reg_pc;
};

AInt PicoRV32::nextFetchedAddress() const {
    return top->rootp->picorv32__DOT__reg_next_pc;
};

void PicoRV32::setProgramCounter(AInt address) {
    top->rootp->picorv32__DOT__reg_pc = address;
};
...
VInt PicoRV32::getRegister(RegisterFileType, unsigned i) const {
    Q_ASSERT(i < 32);
    return top->rootp->picorv32__DOT__cpuregs[i];
};
void PicoRV32::setRegister(RegisterFileType, unsigned i, VInt v) {
    Q_ASSERT(i < 32);
    top->rootp->picorv32__DOT__cpuregs[i] = v;
};

long long PicoRV32::getInstructionsRetired() const {
    return top->rootp->picorv32__DOT__count_instr;
}
long long PicoRV32::getCycleCount() const {
    return top->rootp->picorv32__DOT__count_cycle;
}
