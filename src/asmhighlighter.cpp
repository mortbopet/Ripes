...
QString FieldType::validateField(const QString &field) const {
  const static auto registerRegex =
      QRegularExpression("\\b[(a|s|t|x)][0-9]{1,2}");
  ...
  case Type::Immediate: {
    ...
    if (immediate >= m_lowerBound && immediate <= m_upperBound) {
      ...
      return QString(
                 "Immediate %1 out of valid range; must be within [%2 : %3]")
          .arg(field)
          .arg(m_lowerBound)
          .arg(m_upperBound);
    }
  ...
  case Type::Register: {
    if (!ABInames.contains(field) &&
        !registerRegex.match(field, 0, QRegularExpression::NormalMatch)
             .hasMatch()) {
...
void AsmHighlighter::createSyntaxRules() {
  ...
  types.clear();
  types << FieldType(Type::Register)
        << FieldType(Type::Immediate, -2147483648, 2147483647);
  rule.instr = "li";
  rule.fields = 3;
  rule.inputs = types;
  m_syntaxRules.insert(rule.instr, rule);
