...
void tst_Assembler::tst_matcher() {
    ...
    std::vector<std::pair<QString, unsigned>> toMatch = {
        {"beq", 0b11111100000000000000101011100011},  {"addi", 0b00000111101100010000000010010011},
...
std::vector<std::shared_ptr<Instruction<RVISA>>> tst_Assembler::createInstructions() {
    auto addi = std::shared_ptr<Instruction<RVISA>>(
        new Instruction<RVISA>(Opcode<RVISA>("addi", {OpPart(0b0010011, 0, 6), OpPart(0b000, 12, 14)}),
                               {std::make_shared<Reg<RVISA>>(1, 7, 11), std::make_shared<Reg<RVISA>>(2, 15, 19),
                                std::make_shared<Imm<RVISA>>(3, 12, std::vector{ImmPart(0, 20, 31)})}));
    ...
    auto slli = std::shared_ptr<Instruction<RVISA>>(new Instruction<RVISA>(
        ...
        {std::make_shared<Reg<RVISA>>(1, 7, 11), std::make_shared<Reg<RVISA>>(2, 15, 19),
         std::make_shared<Imm<RVISA>>(3, 5, std::vector{ImmPart(0, 20, 24)})}));
    ...
    auto beq = std::shared_ptr<Instruction<RVISA>>(new Instruction<RVISA>(
        Opcode<RVISA>("beq", {OpPart(0b1100011, 0, 6), OpPart(0b000, 12, 14)}),
        {std::make_shared<Reg<RVISA>>(1, 15, 19), std::make_shared<Reg<RVISA>>(2, 20, 24),
         std::make_shared<Imm<RVISA>>(
             3, 5, std::vector{ImmPart(12, 31, 31), ImmPart(11, 7, 7), ImmPart(5, 25, 30), ImmPart(1, 8, 11)})}));

    return {addi, slti, xori, slli, srai, add, sub, beq};
}
