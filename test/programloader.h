...
/// Utility class around a Ripes EditTab which helps in loading and assembling
/// programs from tests.
...
public:
  ProgramLoader() {
    ...
    connect(m_editTab, &EditTab::programChanged, ProcessorHandler::get(),
            &ProcessorHandler::loadProgram);
  }
...
private:
  // Load a program through the edittab. This is not really suited for automatic
  // testing, since the edit tab will trigger assembling after some timeout. To
  // work around this, we allow for a bit of delay when loading the program.
  void processNewTest() {
