...
Relocation<Reg_T> rv_pcrel_hi() {
  return Relocation<Reg_T>("%pcrel_hi",
...
Relocation<Reg_T> rv_hi() {
  return Relocation<Reg_T>(
      "%hi",
      [](const Reg_T val, const Reg_T /*reloc_addr*/)
          -> HandleRelocationRes<Reg_T> { return {val >> 12 & 0xFFFFFF}; });
}
...
RelocationsVec<Reg_T> rvRelocations() {
  ...
  relocations.push_back(
      std::make_shared<Relocation<Reg_T>>(rv_pcrel_hi<Reg_T>()));
  relocations.push_back(
      std::make_shared<Relocation<Reg_T>>(rv_pcrel_lo<Reg_T>()));
