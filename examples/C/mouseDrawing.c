...
#define MW MOUSE_0_WIDTH
#define MH MOUSE_0_HEIGHT
...
int main() {
    ...
    while (1) {
        ...
        // Scale mouse coordinates to LED matrix and clamp to its bounds.
        int ledX = (MW > 0) ? mouseX * W / MW : 0;
        int ledY = (MH > 0) ? mouseY * H / MH : 0;
        if (ledX < 0) ledX = 0;
        if (ledY < 0) ledY = 0;
        if (ledX >= W) ledX = W - 1;
        if (ledY >= H) ledY = H - 1;
